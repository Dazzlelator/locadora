...
//																// fato, a mesma coisa...
		...
		String paramDestribuidor = request.getParameter("distribuidor");
...
//		String dataCadastro
		...
		System.out.println("json2" + jsonFilmes);
		
		
		if (paramNome != null) {
			Produto produto = ps.getByNome(paramNome);
			Boolean checkNome = (produto != null);
			Boolean checkTipo = (produto.getTipo().equals(paramTipo));

			DateHelper dataLancamento = new DateHelper(paramDataLancamento);
			
			if(checkNome && checkTipo) {
			...
			return "dispatcher:AdicionarFilme"; // lancar um rediretc pra lista de filmes, talvez...
